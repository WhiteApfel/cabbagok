...
class TestConnect:
    ...
    async def test_connect(self, event_loop):
        ...
        mock_transport, mock_protocol = MockTransport(), MockProtocol()
        connection = cabbagok.AmqpConnection(
            hosts=[(HOST, PORT)],
            username=USERNAME,
            password=PASSWORD,
            virtualhost=VIRTUALHOST,
    ...
    async def test_retry(self, event_loop):
        ...
        with patch("cabbagok.amqp.aioamqp_connect", new=faulty_connect), patch(
            "asyncio.sleep"
        ) as mock_sleep:
            await connection.connect()
    ...
    async def test_fatal_error(self, event_loop):
        ...
        with pytest.raises(IncompleteReadError):
            with patch(
                "cabbagok.amqp.aioamqp_connect",
                side_effect=IncompleteReadError([], 160),
            ) as mock_connect:
                await connection.connect()
